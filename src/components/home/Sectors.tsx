...
export default function Sectors() {
  return (
    <section id="sectors" className="py-20 md:py-24 bg-white scroll-mt-28">
      <div className="max-w-7xl mx-auto px-6">
        <SectionTitle subtitle="Eight SolidAI agents, each scoped to a local market workflow — not a generic chatbot.">
          SolidAI sectors
        </SectionTitle>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {SECTORS.map((sector, i) => (
            <Reveal key={i} delay={i * 50} className="glass-card p-5 md:p-6 bg-white border-black/5 text-center group cursor-default hover-lift">
              <div className="text-3xl mb-3" aria-hidden="true">{sector.emoji}</div>
        ...
        <div className="mt-12 text-center">
          <p className="text-slate-600 font-medium max-w-2xl mx-auto leading-relaxed">
            SolidAI helps teams draft, compare, plan, and triage. Expert review stays where it matters.
          </p>
        </div>
      </div>
    </section>
